...
def _extract_parts(parts: list) -> list[dict]:
    ...
    application/json+a2ui or wrapped in <a2a_datapart_json>) become {"kind": "a2ui", "data": <message>} so the UI
    ...
    for p in parts:
        ...
        if text:
            matches = list(re.finditer(r"<a2a_datapart_json>(.*?)</a2a_datapart_json>", text, re.DOTALL))
            if matches:
                last_idx = 0
                for m in matches:
                    prefix = text[last_idx:m.start()].strip()
                    if prefix:
                        out.append({"kind": "text", "text": prefix})
                    last_idx = m.end()
                    try:
                        obj = json.loads(m.group(1))
                        payload = obj.get("data") if isinstance(obj, dict) and "data" in obj else obj
                        out.append({"kind": "a2ui", "data": payload})
                    except Exception:
                        out.append({"kind": "text", "text": m.group(0)})
                suffix = text[last_idx:].strip()
                if suffix:
                    out.append({"kind": "text", "text": suffix})
            else:
                out.append({"kind": "text", "text": text})
        elif data is not None:
            meta = getattr(root, "metadata", None) or {}
            mime = meta.get("mimeType") if isinstance(meta, dict) else None
            if mime == _A2UI_MIME or isinstance(data, dict):
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Poll task until completion if task is still running or parts are empty
        if last_task is not None and getattr(last_task, "id", None):
            ...
            for _ in range(25):
                if parts:
                    break
                try:
                    polled = await a2a_client.get_task(TaskQueryParams(id=task_id))
                    if polled is not None:
                        if getattr(polled, "context_id", None):
                            _contexts[user_id] = polled.context_id
                        if getattr(polled, "history", None):
                            for m in polled.history:
                                if getattr(m, "role", None) in (Role.agent, "agent") and getattr(m, "parts", None):
                                    parts.extend(_extract_parts(m.parts))
                        if getattr(polled, "artifacts", None):
                            for art in polled.artifacts:
                                parts.extend(_extract_parts(art.parts))
                        st = polled.status.state if polled.status else None
                        if st in (TaskState.completed, TaskState.failed, "completed", "failed") or parts:
                            break
                except Exception:
                    pass
